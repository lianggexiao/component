...
public abstract class BaseJCacheTester {
	  ...
	  protected <K, V> boolean simpleGetsAndPutsCacheTest(Cache<K, V> myJCache, int numberOfIteration,
			  int numberOfObjectPerIteration, int sleepTimeMillisBetweenIterations, KeyValueGenerator<K, V> keyValueGenerator) throws InterruptedException {
	    try {
	      LOGGER.info("simpleGetsAndPutsCacheTest BEGIN");
	      if (null != myJCache) {
	        //config check to log  仅仅打印堆内存
	        ...
	        while (iterationCount < numberOfIteration) {
	          ...
	          LOGGER.info("simpleGetsAndPutsCacheTest - Done Iteration #{} of {} in {} micros - total cache ops: {} hits: {} / misses: {}", 
	        		  iterationCount + 1, numberOfIteration, (duration / 1000), opsCount, hitCounts, missCount);
